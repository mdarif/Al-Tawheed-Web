// Sync the Arabic Kitab at-Tawheed matn from the mobile app repo.
//
...
const src = join(appRepo, 'assets', 'content', 'book_tawheed-ar.json');
const dest = join(webRoot, 'src', 'data', 'book_tawheed-ar.json');
...
if (!data?.book?.title || count < 1) {
  console.error('ERROR: source JSON is missing book.title or chapters[]; aborting.');
  process.exit(1);
}

writeFileSync(dest, raw.endsWith('\n') ? raw : raw + '\n');
console.log(`Synced ${count} chapters → src/data/book_tawheed-ar.json`);
